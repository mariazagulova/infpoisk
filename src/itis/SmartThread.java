...
public class SmartThread extends Thread {
	...
	HashMap<String,Double> originvector = new HashMap<String,Double>();
	int start=0; int end=0;
	String[] order = new String[vector.keySet().size()];
	
	public SmartThread(HashMap<String,Double> originvector, HashMap<String,Double> vector, HashMap<String, ListOfProbabilities> matrix, int start, int end, String[] order) {
    ...
    public void run() {
        try {
            for (int i=start; i<=end; i++) {
            	...
            	double old = originvector.get(vectorkey);
    			double sum = 0.15;
    			for (String matrixkey: matrix.keySet()) {
    				if (matrix.get(matrixkey).contains(vectorkey)) {
    					//вот тут умножаем (и сразу в сумму)
    					sum = sum + 0.85*(matrix.get(matrixkey).get(vectorkey) * originvector.get(vectorkey));
    				}
    			}
